...
class compound:
    ...
    def check(cls, print_it=True):
        ...
        if len(not_balanced) > 0:
            if print_it:
                print('not balanced compounds :\r\t\t\t\t',
                      not_balanced)
            return False
        if print_it:
            print('correct balancing : '
                  + 'True')
        return True
...
def show_result(reagents, products):
    for x in reagents:
        print(str(x.factor), str(x.comp), end='')
        if x == reagents[-1]:
            print(' = ', end='')
        else:
            print(' + ', end='')
...
def main(reaction):
    try:
        ...
        compound.clear()
        print(
            f'an error has occured !!! (Error Message : {err})\nplease check the reaction and try again ...')
...
if __name__ == '__main__':
    print(
        '[*] you can split reaction by \
"=" / "=>" / "==>" / "===>" / "→" / "->" / "-->" / "--->" (spaces are not \
necessary)\nenter "q" to exit\n' +
        '[*] example : Hg(OH)2 + H3PO4 = Hg3(PO4)2 + H2O\n' +
        '[*] if you want to see some more examples, enter "test"')
    while 1:
        print('reaction : ', end='')
        reaction = input()
